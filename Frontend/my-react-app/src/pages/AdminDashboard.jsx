...
const AdminDashboard = () => {
  ...
  const handleAddVideo = async () => {
    try {
      ...
      alert("Video Added");
      await fetchVideos(); // ensure updated
      setVideo({ description: "", video_url: "" });
    } catch (err) {
      console.error(err);
    }
  };
  ...
  const handleAddProduct = async () => {
    try {
      ...
      alert("Product Added");
      await fetchProducts(); // ensure updated
  ...
  return (
    <div className="admin-container">
          ...
          <tbody>
            {users.map((user) => (
              <tr key={user.email}>
                ...
                <td>
                  <button className="delete-btn" onClick={() => handleDeleteUser(user.email)}>
                    Delete
                  </button>
                </td>
              </tr>
      ...
      <div className="product-form">
        <h2>Add Video</h2>
        <textarea placeholder="Description" value={video.description} onChange={(e) => setVideo({ ...video, description: e.target.value })} />
      ...
      <div className="products-grid">
        {videos.map((v) => (
          <div className="card" key={v.id}>
            ...
            <p>{v.description}</p>
            <button className="delete-btn" onClick={() => handleDeleteVideo(v.id)}>Delete</button>
          </div>
        ))}
      </div>
    </div>
